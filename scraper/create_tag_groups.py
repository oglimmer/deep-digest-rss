...
def create_tag_groups():
    ...
    response = requests.get(raw_url, headers=config.AUTH_HEADERS, auth=config.AUTH_BASIC, timeout=30)
    ...
    r_patch = requests.patch(patch_url, headers=config.AUTH_HEADERS, auth=config.AUTH_BASIC, json=patch_payload, timeout=30)
