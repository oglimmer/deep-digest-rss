...
public class UserService {
    ...
    public User registerUser(String email, String password) {
        ...
        User user = User.builder().email(email).password(encodedPassword).timezone("Europe/Berlin").build();
        return userRepository.save(user);
    }

    public User authenticateUserAgainstToken(String email, String authToken) {
        User user = userRepository.findByEmail(email).orElseThrow();
        if (user.getAuthToken().equals(authToken) && user.getAuthTokenValidUntil().isAfter(Instant.now())) {
            return user;
        }
        return null;
    }
