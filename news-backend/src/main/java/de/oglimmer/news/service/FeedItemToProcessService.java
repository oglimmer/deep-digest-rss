...
public class FeedItemToProcessService {
  ...
  public FeedItemToProcess getFeedItemToProcessAndMarkAsInProcess() {
    Optional<FeedItemToProcess> feedItemToProcess =
        feedItemToProcessRepository.findFirstByProcessStateOrderByCreatedOnAsc(ProcessState.NEW);
  ...
  public FeedItemToProcess patchFeedItemToProcess(
      Long id, PatchFeedItemToProcessDto patchFeedItemToProcessDto) {
    ...
    switch (feedItemToProcess.getProcessState()) {
      case ProcessState.IN_PROGRESS:
        if (!patchFeedItemToProcessDto.getProcessState().equals(ProcessState.ERROR.name())
          ...
          throw new IllegalArgumentException(
              "Cannot change state from IN_PROGRESS to "
                  + patchFeedItemToProcessDto.getProcessState());
        }
        break;
      ...
      case ProcessState.FAILED:
        throw new IllegalArgumentException(
            "Cannot change state of " + feedItemToProcess.getProcessState());
    }
    feedItemToProcess.setUpdatedOn(Instant.now());
