...
public class ModelMapperConfig {
  ...
  private void confFeedItemToProcess(ModelMapper modelMapper) {
    Converter<Long, Feed> feedConverter =
        context ->
                    ...
                    .orElseThrow(
                        () ->
                            new InvalidDataException(
                                "Feed mit der id " + context.getSource() + " existiert nicht"))
                : null;

    modelMapper.addMappings(
        new PropertyMap<CreateFeedItemToProcessDto, FeedItemToProcess>() {
          @Override
    ...
    modelMapper.addMappings(
        new PropertyMap<FeedItemToProcess, FeedItemToProcessDto>() {
          @Override
          protected void configure() {}
        });
  }
  ...
  private void confNews(ModelMapper modelMapper) {
                    ...
                    .orElseThrow(
                        () ->
                            new InvalidDataException(
                                "Feed mit der id " + context.getSource() + " existiert nicht"))
                : null;
    Converter<Long, FeedItemToProcess> feedItemToProcessConverter =
        context ->
            context.getSource() != null
                ? feedItemToProcessRepository
                    .findById(context.getSource())
                    .orElseThrow(
                        () ->
                            new InvalidDataException(
                                "FeedItemToProcess mit der id "
                                    + context.getSource()
                                    + " existiert nicht"))
                : null;
    Converter<String[], List<Tags>> tagsConverter =
        context -> {
          ...
          return Arrays.stream(context.getSource())
              .map(
                  tagName ->
                      tagsRepository
                          .findByText(tagName)
                          .orElse(Tags.builder().text(tagName).build()))
              .collect(Collectors.toList());
        };
    Converter<List<Tags>, String[]> tagsConverterBack =
        context -> {
          if (context.getSource() == null) {
            return null;
          }
    ...
    modelMapper.addMappings(
        new PropertyMap<CreateNewsDto, News>() {
          @Override
          protected void configure() {
            skip(destination.getId());
            skip(destination.getCreatedOn());
            ...
            using(feedConverter).map(source.getFeedId(), destination.getFeed());
            using(feedItemToProcessConverter)
                .map(source.getOriginalFeedItemId(), destination.getOriginalFeedItem());
            using(tagsConverter).map(source.getTags(), destination.getTags());
          }
        });
    modelMapper.addMappings(
        new PropertyMap<News, NewsDto>() {
          @Override
          protected void configure() {
            //                using(context -> !((News)
            // context.getSource()).getVotes().isEmpty()).map(source, destination.isVoted());
