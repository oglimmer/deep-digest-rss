...
public class NewsUserDetailsService implements UserDetailsService {
    ...
    public NewsUserDetailsService(
            ...
            @Value("${auth.write-user}")
            String writeUser,
            @Value("${auth.write-password}")
            String writePassword,
            @Value("${auth.actuator-user}")
            String actuatorUser,
            @Value("${auth.actuator-password}")
            String actuatorPassword,
            @Value("${auth.swagger-user}")
            String swaggerUser,
            @Value("${auth.swagger-password}")
            String swaggerPassword) {
        this.userRepository = userRepository;
        this.passwordEncoder = passwordEncoder;
        UserDetails read = org.springframework.security.core.userdetails.User.builder()
                .username("read")
                .password(passwordEncoder.encode("read"))
                .roles("READONLY")
                .build();
        UserDetails write = org.springframework.security.core.userdetails.User.builder()
                .username(writeUser)
                .password(passwordEncoder.encode(writePassword))
                .roles("ADMIN", "USER", "READONLY")
                .build();
        UserDetails acuator = org.springframework.security.core.userdetails.User.builder()
                .username(actuatorUser)
                .password(passwordEncoder.encode(actuatorPassword))
                .roles("ADMIN", "USER", "READONLY")
                .build();
        UserDetails swagger = org.springframework.security.core.userdetails.User.builder()
